...
static void check_unicode_value(char32_t c, core_yyscan_t yyscanner);
 ...
 * Reentrant: all state lives on the caller's stack, so concurrent
 * calls from multiple threads are safe.
 */
static List *
od_sql_full_raw_parse(const char *str,
 ...
 */
int
base_yylex(BASE_YYSTYPE *lvalp, YYLTYPE *llocp, core_yyscan_t yyscanner)
{
	...
	if (yyextra->have_lookahead)
	{
		...
		*llocp = yyextra->lookahead_yylloc;
		...
		cur_token = od_sql_full_yylex(&(lvalp->core_yystype), llocp, yyscanner);
	...
	/*
	 * Identify end+1 of current token.  od_sql_full_yylex() has temporarily stored a
	 ...
	 * avoid this by just passing &lookahead_yylloc to od_sql_full_yylex(), but that
	...
	next_token = od_sql_full_yylex(&(yyextra->lookahead_yylval), llocp, yyscanner);
		...
		case USCONST:
			...
			if (next_token == UESCAPE)
			{
				...
				*(yyextra->lookahead_end) = yyextra->lookahead_hold_char;
				...
				/* Get third token */
				next_token = od_sql_full_yylex(&(yyextra->lookahead_yylval),
										llocp, yyscanner);
...
static void
check_unicode_value(char32_t c, core_yyscan_t yyscanner)
{
	if (!is_valid_unicode_codepoint(c))
		scanner_yyerror("invalid Unicode escape value", yyscanner);
}
			  ...
			  int position, core_yyscan_t yyscanner)
{
		...
		if (in[0] == escape)
		{
					 ...
					 isxdigit((unsigned char) in[4]))
			{
				...
				unicode = (hexval(in[1]) << 12) +
					...
					(hexval(in[3]) << 4) +
					hexval(in[4]);
				check_unicode_value(unicode, yyscanner);
				if (pair_first)
				{
					 ...
					 isxdigit((unsigned char) in[7]))
			{
				...
				scanner_yyerror("invalid Unicode escape", yyscanner);
	...
	*out = '\0';
	 ...
	 */
invalid_pair:
	scanner_yyerror("invalid Unicode surrogate pair", yyscanner);
